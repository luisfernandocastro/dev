			...
			$(function(){
				...
				zoomOnScrollSpeed:1, //default value is 3
				 zoomStep:1.1, //default value is 1.6
					...
					markers: [
						{
							latLng: [2.4541666666667, -76.609166666667],
							name: 'Popayán',
						},
						{
		...
		if($('.submitForm').length > 0){
				...
				target.find('input , textarea , select').each(function(){
					if($(this).hasClass('require')){
						if($(this).val().trim() == ''){
							check = 1;
							$(this).focus();
							targetResp.html('Te olvidaste de algo, revisa porfa');
							$(this).addClass('error');
							return false;
						}else{
			...
			$('.submitForm').on('click', function(e) {
				...
				if(check == 0){
					...
					$.ajax({
						url: "https://formspree.io/f/mldlyrdj", 
						method: "POST",
						data: formDetail,
						dataType: "json",
						processData: false,
...
function cambiarAnio(){
	let year_foot = document.getElementById('year_footer');
	let fecha = new Date();

	const year_actual = fecha.getFullYear();

	year_foot.innerHTML = year_actual;
}
